...
def copyrights():
    ...
    cor.grid(row=0,column=0,padx=500,pady=250)
...
def call_wel():
    ...
    for i in l:
        ...
        if u==U and p==P:
            ...
            wel_frame.grid(column=0, row=0,padx=380,pady=200)
            return
        elif i==l[len(l)-1]:
            user_entry.delete(0,END)
            passw_entry.delete(0,END)
...
def BkTF():
    ...
    welcome_frame.grid(row=0,column=0,pady=200,padx=270)
...
def call_sign():
    sign_frame.grid(column=0, row=0,padx=370,pady=200)
    welcome_frame.grid_forget()
    
def chem_menu():
    wel_frame.grid_forget()
    chem_frame.grid(row=0,column=0,padx=320,pady=100)

def history():
    wel_frame.grid_forget()
    his.grid(row=0,column=0,padx=350,pady=100)

def register():
    sign2_frame.grid(column=0, row=0,padx=530,pady=80)
    welcome_frame.grid_forget()
...
d=str(datetime.date.today())
a=d[0:4]
cr_lab2=Label(cor,text='Copyright (c) 2019-'+a+' Vinit Aggarwal',
